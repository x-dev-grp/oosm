#!/usr/bin/env node
/**
 * Bump OOSM semver from VERSION file and sync Maven + version.properties.
 *
 * Usage:
 *   node scripts/version/bump-version.cjs patch|minor|major|set 1.2.3
 *   node scripts/version/bump-version.cjs patch --no-changelog
 */
const fs = require('fs');
...
function syncMaven(version) {
  try {
    execSync(`mvn -q versions:set -DnewVersion=${version} -DgenerateBackupPoms=false -DprocessAllModules`, {
      cwd: repoRoot,
      stdio: 'inherit'
    });
    execSync('mvn -q versions:commit', { cwd: repoRoot, stdio: 'inherit' });
  } catch (err) {
    console.warn('mvn versions:set failed — falling back to VERSION + version.properties only.');
  }
}

function writeVersionProperties(version) {
  const propsPath = path.join(repoRoot, 'app/src/main/resources/version.properties');
  let gitSha = 'local';
  try {
