...
public class PermissionService extends BaseServiceImpl<Permission, PermissionDTO, PermissionDTO> {
    ...
    protected PermissionService(BaseRepository<Permission> repository, ModelMapper modelMapper, PermissionRepository permissionRepository) {
        ...
        OOSMLogger.logMethodEntry(this.getClass(), "PermissionService", "Initializing PermissionService");
        
        try {
            this.permissionRepository = permissionRepository;
            
            OOSMLogger.logMethodExit(this.getClass(), "PermissionService", "PermissionService initialized successfully");
            OOSMLogger.logPerformance(this.getClass(), "PermissionService", startTime, System.currentTimeMillis());
            OOSMLogger.logSecurityEvent(this.getClass(), "PERMISSION_SERVICE_INITIALIZED", 
                "Permission service initialized successfully");
            
        } catch (Exception e) {
            OOSMLogger.logException(this.getClass(), "Error initializing PermissionService", e);
            throw e;
        }
    }
