...
import com.xdev.ooms.sharedkernel.mail.templates.OosmMailComposer;
...
@Service
public class UserService extends BaseServiceImpl<OOSMUser, OOSMUserDTO, OOSMUserOUTDTO> implements UserDetailsService {
    ...
    private static final String USER_MGMT_ENTITY = "OOSMUSER";

    private final UserRepository userRepository;
    private final MailService mailService;
    private final OosmMailComposer mailComposer;
    private final ConfirmationCodeService confirmationCodeService;
    private final PasswordEncoder passwordEncoder;
    private final RoleRepository roleRepository;
    private final CompanyProfileRepository companyProfileRepository;

    protected UserService(BaseRepository<OOSMUser> repository, ModelMapper modelMapper, UserRepository userRepository, MailService mailService, OosmMailComposer mailComposer, ConfirmationCodeService confirmationCodeService, PasswordEncoder passwordEncoder, RoleRepository roleRepository, CompanyProfileRepository companyProfileRepository) {
        super(repository, modelMapper);

        long startTime = System.currentTimeMillis();
        OOSMLogger.logMethodEntry(this.getClass(), "UserService", "Initializing UserService");
        ...
        try {
            ...
            OOSMLogger.logMethodExit(this.getClass(), "UserService", "UserService initialized successfully");
            OOSMLogger.logPerformance(this.getClass(), "UserService", startTime, System.currentTimeMillis());
            OOSMLogger.logSecurityEvent(this.getClass(), "USER_SERVICE_INITIALIZED",
                    "User service initialized successfully");

        } catch (Exception e) {
            OOSMLogger.logException(this.getClass(), "Error initializing UserService", e);
            throw e;
        }
    ...
    @Override
    public SearchResponse<OOSMUser, OOSMUserOUTDTO> search(SearchData searchData) {
        assertCanListUsers();
        SearchResponse<OOSMUser, OOSMUserOUTDTO> response = super.search(searchData);
    ...
    public OOSMUserOUTDTO findById(UUID id) {
        OOSMUser target = userRepository.findByIdAndIsDeletedFalse(id)
        ...
        OOSMUserOUTDTO result = modelMapper.map(target, OOSMUserOUTDTO.class);
        enrichTenantNames(List.of(result));
        stripPhotoPayloads(List.of(result));
        return result;
    }

    private void stripPhotoPayloads(List<OOSMUserOUTDTO> users) {
        if (users == null) {
            return;
        }
        for (OOSMUserOUTDTO user : users) {
            if (user == null) {
                continue;
            }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<OOSMUserOUTDTO> findAll() {
        assertCanListUsers();
        List<OOSMUserOUTDTO> users = super.findAll();
        enrichTenantNames(users);
        stripPhotoPayloads(users);
        return users;
    }

    private void enrichTenantNames(List<OOSMUserOUTDTO> users) {
        if (users == null || users.isEmpty()) {
            return;
        }

        Set<UUID> tenantIds = users.stream()
                .map(OOSMUserOUTDTO::getTenantId)
    ...
    public OOSMUserOUTDTO addUser(OOSMUserOUTDTO userDTO) throws Exception {
        long startTime = System.currentTimeMillis();
        ...
        try {
            ...
            OOSMUserOUTDTO result = modelMapper.map(savedUser, OOSMUserOUTDTO.class);
            if (!mailEnabled) {
    ...
    @Transactional(rollbackFor = Exception.class)
    public OOSMUserOUTDTO updateUser(OOSMUserOUTDTO userDTO, UUID id) throws Exception {
        long startTime = System.currentTimeMillis();
        String username = userDTO != null ? userDTO.getUsername() : "null";
        ...
        try {
            ...
            OOSMUser user = repository.findByIdAndIsDeletedFalse(id)
            ...
            if (usernameChanged || emailChanged || phoneChanged) {
                ...
                userRepository.save(user);
                dispatchConfirmationAsync(userDTO, rawPassword);
                OOSMLogger.logSecurityEvent(this.getClass(), "USER_CREDENTIALS_UPDATED",
            ...
            return modelMapper.map(user, OOSMUserOUTDTO.class);
    ...
    private void sendConfirmation(OOSMUserOUTDTO userDTO, String rawPassword) throws Exception {
        long startTime = System.currentTimeMillis();
        String username = userDTO != null ? userDTO.getUsername() : "null";
        OOSMLogger.logMethodEntry(this.getClass(), "sendConfirmation",
        ...
        try {
            switch (userDTO.getConfirmationMethod()) {
                case EMAIL -> {
                    ...
                    OOSMLogger.logSecurityEvent(this.getClass(), "CONFIRMATION_EMAIL_SENT",
                            "Confirmation email sent to: " + userDTO.getEmail());
                }
                case PHONE -> {
                    //TODO send sms message
                    OOSMLogger.logSecurityEvent(this.getClass(), "CONFIRMATION_SMS_PENDING",
            ...
            OOSMLogger.logMethodExit(this.getClass(), "sendConfirmation",
                    "Confirmation sent successfully for user: " + username);
            OOSMLogger.logPerformance(this.getClass(), "sendConfirmation", startTime, System.currentTimeMillis());
    ...
    @Transactional(rollbackFor = Exception.class)
    public OOSMUserOUTDTO resetPassword(String identifier) throws Exception {
        long startTime = System.currentTimeMillis();
        OOSMLogger.logMethodEntry(this.getClass(), "resetPassword", "Password reset request for identifier: " + identifier);

        try {
            OOSMUser user = userRepository.findByPhoneOrEmailIgnoreCase(identifier).orElse(null);
            if (user != null) {
                if (user.isLocked()) {
                ...
                if (user.getEmail() != null && user.getEmail().equalsIgnoreCase(identifier)) {
                    ...
                    OOSMLogger.logSecurityEvent(this.getClass(), "PASSWORD_RESET_CODE_SENT",
                            "Password reset code sent to email: " + user.getEmail());
                } else {
                    //TODO send to phone number
                    OOSMLogger.logSecurityEvent(this.getClass(), "PASSWORD_RESET_SMS_PENDING",
                            "Password reset SMS pending for phone: " + identifier);
                }

                OOSMLogger.logMethodExit(this.getClass(), "resetPassword",
                ...
                OOSMLogger.logSecurityEvent(this.getClass(), "PASSWORD_RESET_INITIATED",
                        "Password reset initiated successfully for identifier: " + identifier);

                return modelMapper.map(user, OOSMUserOUTDTO.class);

            } else {
                OOSMLogger.logSecurityEvent(this.getClass(), "PASSWORD_RESET_INVALID_IDENTIFIER",
    ...
    private ConfirmationCode saveConfirmationCode(ConfirmationCode code) {
        long startTime = System.currentTimeMillis();
        OOSMLogger.logMethodEntry(this.getClass(), "saveConfirmationCode",
        ...
        try {
            ...
            if (existedCode != null) {
                ...
                OOSMLogger.logMethodExit(this.getClass(), "saveConfirmationCode", "Existing confirmation code updated");
                OOSMLogger.logPerformance(this.getClass(), "saveConfirmationCode", startTime, System.currentTimeMillis());
            ...
            OOSMLogger.logMethodExit(this.getClass(), "saveConfirmationCode", "New confirmation code saved");
            OOSMLogger.logPerformance(this.getClass(), "saveConfirmationCode", startTime, System.currentTimeMillis());
    ...
    public boolean validateResetCode(String code, UUID userId) throws Exception {
        ...
        try {
            OOSMUser user = userRepository.findByIdAndIsDeletedFalse(userId).orElse(null);
            if (user == null) {
                OOSMLogger.logSecurityEvent(this.getClass(), "RESET_CODE_VALIDATION_USER_NOT_FOUND",
            ...
            if (existedCode == null || existedCode.isConsumed() || existedCode.getFailedAttempts() >= 3) {
                OOSMLogger.logSecurityEvent(this.getClass(), "RESET_CODE_VALIDATION_INVALID",
                        "Reset code validation failed - Invalid code for user: " + userId);
                throw new IllegalArgumentException("Invalid code");
            }
            if (existedCode.isExpired()) {
                OOSMLogger.logSecurityEvent(this.getClass(), "RESET_CODE_VALIDATION_EXPIRED",
            ...
            OOSMLogger.logMethodExit(this.getClass(), "validateResetCode",
    ...
    public void updatePassword(UpdatePasswordDTO dto, UUID userId) {
        ...
        try {
            OOSMUser user = userRepository.findByIdAndIsDeletedFalse(userId).orElse(null);
            if (user == null) {
                OOSMLogger.logSecurityEvent(this.getClass(), "PASSWORD_UPDATE_USER_NOT_FOUND",
            ...
            OOSMLogger.logMethodExit(this.getClass(), "updatePassword",
    ...
    public OOSMUserOUTDTO getCurrentUserProfile(String username) {
        OOSMUser user = userRepository.findByUsernameAndIsDeletedFalse(username)
                .orElseThrow(() -> new UsernameNotFoundException(username));
        OOSMUserOUTDTO dto = modelMapper.map(user, OOSMUserOUTDTO.class);
    ...
    @Transactional(rollbackFor = Exception.class)
    public OOSMUserOUTDTO updateCurrentUserProfile(String username, UserProfileUpdateDTO profileDto) {
        if (profileDto == null) {
        ...
        OOSMUserOUTDTO validationDto = new OOSMUserOUTDTO();
        ...
        OOSMUserOUTDTO result = modelMapper.map(user, OOSMUserOUTDTO.class);
        enrichTenantNames(List.of(result));
    ...
    private void validateUserDTO(OOSMUserOUTDTO userDTO) {
        if (userDTO == null) {
            OOSMLogger.logSecurityEvent(this.getClass(), "USER_VALIDATION_NULL_DTO",
                    "User validation failed - DTO is null");
            throw new IllegalArgumentException("User data must not be null");
        }
        if (userDTO.getUsername() == null || userDTO.getUsername().isBlank()) {
            OOSMLogger.logSecurityEvent(this.getClass(), "USER_VALIDATION_MISSING_USERNAME",
                    "User validation failed - Username is missing");
            throw new IllegalArgumentException("Username is required");
        }
        if (userDTO.getConfirmationMethod() == null) {
            OOSMLogger.logSecurityEvent(this.getClass(), "USER_VALIDATION_MISSING_CONFIRMATION_METHOD",
        ...
        if (userDTO.getConfirmationMethod() == ConfirmationMethod.EMAIL && (userDTO.getEmail() == null || userDTO.getEmail().isBlank())) {
            OOSMLogger.logSecurityEvent(this.getClass(), "USER_VALIDATION_MISSING_EMAIL",
        ...
        if (userDTO.getConfirmationMethod() == ConfirmationMethod.PHONE && (userDTO.getPhoneNumber() == null || userDTO.getPhoneNumber().isBlank())) {
            OOSMLogger.logSecurityEvent(this.getClass(), "USER_VALIDATION_MISSING_PHONE",
    ...
    public String generateSecureCode(int length) {
        long startTime = System.currentTimeMillis();
        OOSMLogger.logMethodEntry(this.getClass(), "generateSecureCode", "Generating secure code with length: " + length);
        ...
        try {
            ...
            OOSMLogger.logMethodExit(this.getClass(), "generateSecureCode", "Secure code generated successfully");
            OOSMLogger.logPerformance(this.getClass(), "generateSecureCode", startTime, System.currentTimeMillis());
            OOSMLogger.logSecurityEvent(this.getClass(), "SECURE_CODE_GENERATED",
    ...
    public String generateRandomCode() {
        long startTime = System.currentTimeMillis();
        OOSMLogger.logMethodEntry(this.getClass(), "generateRandomCode", "Generating random 6-digit code");

        try {
            int code = SecureRandomHolder.INSTANCE.nextInt(900_000) + 100_000;
            String codeString = String.valueOf(code);

            OOSMLogger.logMethodExit(this.getClass(), "generateRandomCode", "Random code generated successfully");
            OOSMLogger.logPerformance(this.getClass(), "generateRandomCode", startTime, System.currentTimeMillis());
            OOSMLogger.logSecurityEvent(this.getClass(), "RANDOM_CODE_GENERATED",
    ...
    public List<OOSMUserDTO> findByRoleName(String roleName) {

        long startTime = System.currentTimeMillis();
        OOSMLogger.logMethodEntry(this.getClass(), "findByRoleName", "Finding users by role name: " + roleName);

        try {
            List<OOSMUserDTO> users = userRepository.findByRoleRoleNameAndTenantIdAndIsDeletedFalse(roleName, TenantContext.getCurrentTenant()).stream().map(
                    user -> modelMapper.map(user, OOSMUserDTO.class)
            ).toList();

            OOSMLogger.logMethodExit(this.getClass(), "findByRoleName",
                    "Found " + users.size() + " users with role: " + roleName);
            OOSMLogger.logPerformance(this.getClass(), "findByRoleName", startTime, System.currentTimeMillis());
            OOSMLogger.logDataAccess(this.getClass(), "READ_BY_ROLE", "OOSMUser");
    ...
    public List<OOSMUserDTO> findByRole(String roleName) {
        UUID tenantId = TenantContext.getCurrentTenant();
        List<OOSMUser> users = userRepository.findByRoleNameAndTenant(roleName, tenantId);
        return users.stream()
                .map(user -> modelMapper.map(user, OOSMUserDTO.class))
                .toList();
    }
    ...
    @Override
    public Set<Action> actionsMapping(OOSMUser user) {
    ...
    private void assertCanViewUser(OOSMUser target) {
        if (!hasUserManagementPermission("READ")) {
    ...
    private void assertCanManageUser(OOSMUser target) {
    ...
    private void assertSameTenant(OOSMUser target) {
