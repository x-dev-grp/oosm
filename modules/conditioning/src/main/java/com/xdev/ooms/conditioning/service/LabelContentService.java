...
import com.xdev.ooms.conditioning.model.LabelSource;
import com.xdev.ooms.conditioning.repository.LabelContentRepository;
import com.xdev.ooms.conditioning.repository.CertificationRepository;
...
public class LabelContentService {
    ...
    public LabelContentService(
            ...
            ConditioningProductionSupport productionSupport,
    ...
    public LabelContentDto generate(LabelGenerateRequestDto request) {
        ...
        StorageUnitDto storageUnit = productionSupport.getStorageUnit(request.getLotId());
        GenealogyDto genealogy = fetchGenealogy(request.getLotId());
        ProduitFinalDto packaging = inventorySupport.getProduitFinalById(request.getPackagingId());
        ...
        CompanyProfileDto companyProfile = companyProfileRepository.findById(tenantId)
                .map(profile -> modelMapper.map(profile, CompanyProfileDto.class))
                .orElse(null);
    ...
    private ProduitFinalDto loadPackaging(UUID packagingId) {
        try {
            return inventorySupport.getProduitFinalById(packagingId);
    ...
    private void saveSourceProofs(
        ...
        if (filtrationOperationId != null) {
            try {
                Object operation = productionSupport.getFiltrationOperation(filtrationOperationId);
                if (operation != null) {
                    addSnapshot(labelContent, LabelSourceType.FILTRATION_OPERATION, filtrationOperationId, "OP-" + filtrationOperationId.toString().substring(0, 8), operation);
                }
            } catch (Exception e) {
