...
@Component
public class OilBonCommandePdfConfigMapper {
    ...
    public OilBonCommandePdfConfigMapper(OilContainerSaleRepo containerSaleRepo, StorageUnitRepo storageUnitRepo) {
    ...
    public FormPdfConfigDto map(OilSale sale) {
        ...
        String documentDate = formatDate(sale.getSaleDate() != null ? sale.getSaleDate() : sale.getCreatedDate());

        FormPdfConfigDto config = new FormPdfConfigDto();
        config.setTitle(FormPdfLabels.GEN_PDF_BON_COMMANDE);
        config.setReference("FOR-COM-01");
